...
using TulparUI.Properties;

namespace TulparUI
{
    public class TulparSkinManager
    {
        private static TulparSkinManager _instance;

        private readonly List<TulparForm> _formsToManage = new List<TulparForm>();
        ...
        public static TulparSkinManager Instance => _instance ?? (_instance = new TulparSkinManager());

        public int FORM_PADDING = 14;

        // Constructor
        private TulparSkinManager()
        {
            Theme = Themes.LIGHT;
            ...
            logicalFonts.Add("Subtitle", CreateLogicalFont("Roboto", 12, NativeTextRenderer.logFontWeight.FW_REGULAR));
            ...
            logicalFonts.Add("Button2", CreateLogicalFont("Roboto Medium", 12, NativeTextRenderer.logFontWeight.FW_MEDIUM));
        ...
        public Font getFontByType(FontType type)
        {
            switch (type)
            {
                ...
                case FontType.Button2:
                    return new Font(RobotoFontFamilies["Roboto"], 12f, FontStyle.Bold, GraphicsUnit.Pixel);
        ...
        // Dyanmic Themes
        public void AddFormToManage(TulparForm materialForm)
        {
        ...
        public void RemoveFormToManage(TulparForm materialForm)
        {
        ...
        private void UpdateControlBackColor(Control controlToUpdate, Color newBackColor)
        {
            ...
            // Material Divider
            else if (controlToUpdate is TulparDivider)
            {
                controlToUpdate.BackColor = DividersColor;
            }

            // Other Material Skin control
            else if (controlToUpdate.IsTulparControl())
            {
            ...
            else if (EnforceBackcolorOnAllComponents && controlToUpdate.HasProperty("BackColor") && !controlToUpdate.IsTulparControl() && controlToUpdate.Parent != null)
            {
                controlToUpdate.BackColor = controlToUpdate.Parent.BackColor;
                controlToUpdate.ForeColor = TextHighEmphasisColor;
                controlToUpdate.Font = getFontByType(TulparSkinManager.FontType.Body1);
            }
        ...
        private void UpdateToolStrip(ToolStrip toolStrip, Color newBackColor)
        {
            ...
            foreach (ToolStripItem control in toolStrip.Items)
            {
                ...
                if (control is TulparToolStripMenuItem && (control as TulparToolStripMenuItem).HasDropDown)
                {
                    //recursive call
                    UpdateToolStrip((control as TulparToolStripMenuItem).DropDown, newBackColor);
                }
            }
        }
    }
